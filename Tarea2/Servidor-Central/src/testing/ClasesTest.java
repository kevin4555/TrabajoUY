...
public class ClasesTest {
  ...
  public void paquetePublicacionClase()
      throws PaquetePublicacionYaFueComprado, TipoDePublicacionYaFueIngresado, IOException {
    ...
    paquetePublicacion.agregarTipoPublicacion(tipoPublicacionInicial, 2);
    try {
      paquetePublicacion.agregarTipoPublicacion(tipoPublicacionInicial, 2);
    } catch (TipoDePublicacionYaFueIngresado e) {
      Assert.assertEquals(
          "El tipo de publicación " + tipoPublicacionInicial.getNombre() + " ya fue ingresado",
          e.getMessage());
    }
    
    paquetePublicacion.setEstaComprado(true);
    Assert.assertEquals(paquetePublicacion.getEstaComprado(), true);
    try {
      paquetePublicacion.agregarTipoPublicacion(tipoPublicacionInicial, 2);
    } catch (PaquetePublicacionYaFueComprado e) {
      Assert.assertEquals("El paquete " + paquetePublicacion.getNombre() + " ya fue comprado",
          e.getMessage());
    }
    ...
    DtpaquetePublicacion dtpaquetePublicacion = paquetePublicacion.obtenerDtPaquete();
    
    Assert.assertEquals(dtpaquetePublicacion.getNombre(), paquetePublicacion.getNombre());
    Assert.assertEquals(dtpaquetePublicacion.getDescripcion(),
        paquetePublicacion.getDescripcion());
    Assert.assertEquals(dtpaquetePublicacion.getFechaAlta(),
        paquetePublicacion.getFechaAlta());
    
    Assert.assertEquals(String.valueOf(costo), String.valueOf(paquetePublicacion.getCosto()));
    Assert.assertEquals(String.valueOf(paquetePublicacion.getDescuento()),
        String.valueOf(50f));
    ...
    Assert.assertEquals(resultadoList.get(1), "tipoTestingInicial");
  }
  ...
  public void tipoPublicacionClase() {
    TipoPublicacion tipoPublicacion = new TipoPublicacion("tipoTesting", "Uso para testing",
        "baja", 50, 500f, fechaDate);
    DttipoPublicacion dttipoPublicacion = tipoPublicacion.obtenerDttipoPublicacion();
    Assert.assertEquals(dttipoPublicacion.getDescripcion(), tipoPublicacion.getDescripcion());
    Assert.assertEquals(dttipoPublicacion.getExposicion(), tipoPublicacion.getExposicion());
    Assert.assertEquals(dttipoPublicacion.getNombre(), tipoPublicacion.getNombre());
    Assert.assertEquals(String.valueOf(dttipoPublicacion.getCosto()),
        String.valueOf(tipoPublicacion.getCosto()));
    Assert.assertEquals(dttipoPublicacion.getDuracionDia(), tipoPublicacion.getDuracionDia());
    Assert.assertEquals(dttipoPublicacion.getFechaAlta(), tipoPublicacion.getFechaAlta());
  }
  
  @Test
  public void postulanteClase() throws IOException {
    Postulante postulante = new Postulante("NicknameTest", "NombreTest", "ApellidoTest",
        "EmailTest", fechaDate, "NacionalidadTest", null, "1234");
    ...
    OfertaLaboral ofertaLaboral = new OfertaLaboral("test", "descipcionTest", "09:00", "15:00",
        500f, "Montevideo", "Montevideo", fechaDate, tipoPublicacion, null, empresa);
    ...
    List<Postulacion> postulacionList = postulante.getPostulaciones();
    
    Assert.assertEquals(dtOfertaLaboral.get(0).getNombre(), ofertaLaboral.getNombre());
    Assert.assertEquals(dtOfertaLaboral.get(0).getEstadoOferta(), ofertaLaboral.getEstado());
    
    Assert.assertEquals(postulacionList.get(0).getCvReducido(), postulacion.getCvReducido());
    Assert.assertEquals(postulacionList.get(0).getNombreOfertaLaboral(),
        postulacion.getNombreOfertaLaboral());
    Assert.assertEquals(postulacionList.get(0).getDescrpMotivacion(),
        postulacion.getDescrpMotivacion());
    Assert.assertEquals(postulacionList.get(0).getPostulante(), postulacion.getPostulante());
  }
  ...
  public void compraPaqueteClase() {
    ...
    CompraPaquete compraPaquete = new CompraPaquete(fechaDate, paquetePublicacion);
    
    LocalDate copiaFechaCompra = LocalDate.of(fechaDate.getYear(), fechaDate.getMonth(),
        fechaDate.getDayOfMonth());
    LocalDate fechaVencimiento = copiaFechaCompra
        .plusDays(paquetePublicacion.getPeriodoValidez());
    
    Assert.assertEquals(compraPaquete.getFechaCompra(), fechaDate);
    Assert.assertEquals(compraPaquete.getFechaVencimiento(), fechaVencimiento);
  }
  
  @Test
  public void empresaClase() throws OfertaLaboralYaExisteException, IOException {
    ...
    try {
      ...
      Assert.assertEquals(e.getMessage(), "La Oferta Laboral " + ofertaLaboral.getNombre()
          + " ya esta asociada a la Empresa " + empresa.getNickname());
    ...
    List<String> nombreOferta = empresa.listarNombreOfertasUsuario();
    Assert.assertEquals(nombreOferta.get(0), ofertaLaboral.getNombre());
    ...
    Assert.assertEquals(empresa.estaCompradoPaquete(paquetePublicacion.getNombre()), true);
  ...
  public void ofertaLaboralClase() throws IOException {
    ...
    Assert.assertEquals(ofertaLaboral.getDescripcion(), "nuevaDescripcion");
    ofertaLaboral.setCiudad("Soriano");
    Assert.assertEquals(ofertaLaboral.getCiudad(), "Soriano");
    ofertaLaboral.setDepartamento("Soriano");
    Assert.assertEquals(ofertaLaboral.getDepartamento(), "Soriano");
    ofertaLaboral.setRemunaracion(50f);
    Assert.assertEquals(String.valueOf(ofertaLaboral.getRemunaracion()), String.valueOf(50f));
    ofertaLaboral.setFechaAlta(fechaDate);
    Assert.assertEquals(ofertaLaboral.getFechaAlta(), fechaDate);
    ofertaLaboral.setKeyword(keywordsList);
    Assert.assertEquals(ofertaLaboral.getKeywords(), keywordsList);
    ofertaLaboral.setTipoPublicacion(tipoPublicacion);
    Assert.assertEquals(ofertaLaboral.getTipoPublicacion(), tipoPublicacion);
    Assert.assertEquals(ofertaLaboral.getPostulacion(), new ArrayList<Postulacion>());
    Assert.assertEquals(ofertaLaboral.getCompraPaquete(), null);
    ofertaLaboral.setHorarioFinal(fecha);
    Assert.assertEquals(ofertaLaboral.getHorarioFinal(), fecha);
    ofertaLaboral.setHorarioInicial(fecha);
    Assert.assertEquals(ofertaLaboral.getHorarioInicial(), fecha);
    ...
    Assert.assertEquals(dtOfertaLaboral.getNombre(), ofertaLaboral.getNombre());
    Assert.assertEquals(dtOfertaLaboral.getDescripcion(), ofertaLaboral.getDescripcion());
    Assert.assertEquals(dtOfertaLaboral.getPaqueteAsociado().getNombre(),
