...
public class CompraPaqueteServlet extends HttpServlet {
	...
	private void procesarRequest(HttpServletRequest request, HttpServletResponse response)
		...
		main.java.webservices.Publicador port = publicadorService.getPublicadorPort();
		String nicknameempresa = request.getParameter("nicknameEmpresa");
		String nombrePaquete = request.getParameter("nombrePaquete");
		try {
			ArrayList<String> paquetesNoComprados = (ArrayList<String>) port
					.listarPaquetesNoCompradosDeEmpresa(nicknameempresa).getItem();
			...
			if (permitido) {
				...
				String url = request.getContextPath() + "/perfil?nicknameUsuario=" + nicknameempresa;
				response.sendRedirect(url);
				return;
			} else {
				String url = request.getContextPath() + "/paquete?nombrePaquete="
						+ URLEncoder.encode(nombrePaquete, "UTF-8") + "&error="
						+ URLEncoder.encode("Paquete ya comprado", "UTF-8");
				response.sendRedirect(url);
				return;
			}
		} catch (Exception e) {
