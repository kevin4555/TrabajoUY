...
public class SeguirDejarSeguirServlet extends HttpServlet {
	...
	private void procesarRequest(HttpServletRequest request, HttpServletResponse response)
		...
		boolean seguir = !usuarioLogueado.getSeguidos().contains(nicknameSeguido);
		main.java.webservices.PublicadorService service = new PublicadorService();
		main.java.webservices.Publicador port = service.getPublicadorPort();
		Boolean flag = false;

		try {
			if (seguir) {
				port.agregarSeguidor(nicknameSeguido, usuarioLogueado.getNickname());
				DtUsuario usuarioActualizado = port.obtenerDtUsuario(usuarioLogueado.getNickname());
		...
		} catch (UsuarioNoExisteException_Exception | IOException_Exception | UsuarioNoEstaSeguidoException_Exception
