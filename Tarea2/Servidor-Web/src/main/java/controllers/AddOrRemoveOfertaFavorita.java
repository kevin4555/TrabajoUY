...
public class AddOrRemoveOfertaFavorita extends HttpServlet {
    ...
    private void procesarRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	...
    	if(!sesion.getAttribute("estadoSesion").equals(EstadoSesion.LOGIN_CORRECTO) || !sesion.getAttribute("tipoUsuario").equals(TipoUsuario.POSTULANTE)) {
    		response.sendError(403, "No autorizado");
    		return;
    	}
    	DtPostulante dtPostulante = (DtPostulante) sesion.getAttribute("usuarioLogueado");
      ...
      logica.webservices.Publicador port = publicadorService.getPublicadorPort();
    	...
    	try {
    	    if (dtPostulante.getOfertasFavoritas().contains(nombreOferta)) {
    	        port.removerOfertaFavorita(nicknamePostulante, nombreOferta);
    	        dtPostulante.getOfertasFavoritas().remove(nombreOferta);
    	    } else {
    	        port.agregarOfertaFavorita(nicknamePostulante, nombreOferta);
    	        dtPostulante.getOfertasFavoritas().add(nombreOferta);
    	    }
